function init(element) {
    ...
    search_div.classList.add("search-container-custom");
...
function addPlaceholder(wrapper) {
    ...
    if (!tokens.length && !(document.activeElement === input_search))
        input_search.setAttribute("placeholder", "Please Choose Country");
}
...
function createToken(wrapper, value) {
    const search = wrapper.querySelector(".search-container-custom");
...
function populateAutocompleteList(select, query, dropdown = false) {
    ...
    const wrapper = select.parentNode;
    const input_search = wrapper.querySelector(".search-container-custom");
    const autocomplete_list = wrapper.querySelector(".autocomplete-list");
    autocomplete_list.innerHTML = "";
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // var multiSelectComponent = document.querySelector(".multi-select-component");
    // var multiParent = multiSelectComponent.childNodes[0];
    // multiSelectComponent.addEventListener("keypress", function(event) {
    //     if (event.key === "Enter" && event.target.value != "") {
    //         event.preventDefault();
    //         var opt = document.createElement('option');
    //         opt.value = event.target.value;
    //         opt.innerHTML = event.target.value;
    //         multiParent.appendChild(opt);
    //     }
    // })
});


// @props(['title', 'required' => false, 'name', 'multipleSelect' => false])
// <x-form.control>
//     <x-form.label :title="$title" :required="$required" />
//     @if ($multipleSelect)
//         <select class="{{ config('config.sampleForm.selectBox') }}" name="{{ $name }}" multiple
...
{/* <x-form.label_group title="user.country" name="country[]" :multipleSelect="true" :required="true">
@foreach ($viewCountries as $c)
    <x-form.option :title="$c['name']" :value="$c['name']" field="country" />
@endforeach
</x-form.label_group> */}
